...
def calibrate_mag(samples, delay, boards, bias_gyro):
    ...
    for j in range(samples): #Läs ut data under x antal samples och spara resultat i en vektor 
        ...
        dt = now - last_time
        last_time = time.monotonic()
        gyro_angle += np.degrees(gyro_z * dt)
        print(dt)
...
def get_imu(queue, calibration_angle):
    ...
    mag.data_rate = Rate_mag.RATE_20_HZ #Sätter även PerformanceMode till MODE_ULTRA
    mag.range = Mag_range.RANGE_4_GAUSS

    
    #mag.range = LIS3MDL_RANGE_4_GAUSS
    ...
    last_time = time.monotonic()
    fused_yaw = angle_offset + calibration_angle
    filtered_unwrapped_mag = angle_offset + calibration_angle
    previous_unwrapped = angle_offset + calibration_angle
    previous_heading = angle_offset + calibration_angle
    while True: 
        ...
        unwrapped_mag = unwrap_heading(angle_deg, previous_heading, previous_unwrapped)
        previous_heading = angle_deg
        previous_unwrapped = unwrapped_mag

        #Complementary filter
        filtered_unwrapped_mag = alpha_mag * filtered_unwrapped_mag + (1 - alpha_mag) * unwrapped_mag
        alpha = compute_alpha(result_gyro[2]) #Beräknar alpha värdet baserat på rotationshastigheten
        
        now = time.monotonic()
        dt = now - last_time
        last_time = now
        #print("Rotationshastighet: ", result_gyro[2])
        if abs(result_gyro[2]) > 0.08: 
            gyro_yaw = fused_yaw - np.degrees(gyro_z * dt)
        else:
            gyro_yaw = fused_yaw
